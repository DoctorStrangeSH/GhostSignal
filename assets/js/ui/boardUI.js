class BoardUI {
    ...
    render() {
        ...
        this.container.innerHTML = `
            ...
            <div class="board-canvas" id="board-canvas">
                ...
                <div class="board-slots" id="board-slots">
                    ${this._renderSlots()}
                </div>
            </div>
            ...
            <div class="board-evidence-pool">
                ...
                <div class="evidence-pool-items" id="evidence-pool">
                    ${this._renderEvidencePool()}
                </div>
            </div>
        `;
    ...
    _renderEvidencePool() {
        ...
        if (evidenceObjects.length === 0) {
            return '<div class="pool-empty">Все улики на доске</div>';
        }
        
        return evidenceObjects.map(ev => `
            <div class="evidence-pool-item" 
                 draggable="true" 
    ...
    _bindEvents() {
        ...
        // Кнопки удаления улики из слота
        this.container.querySelectorAll('.slot-remove-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                e.stopPropagation();
                const slotId = btn.dataset.slot;
                if (slotId && this.board) {
                    this.board.removeEvidence(slotId);
                }
            });
        });
            ...
            if (slotFrom && slotTo) {
                ...
                linesHTML += `
                    <line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}"
                          ...
                          stroke-dasharray="${conn.type === 'dashed' ? '6,4' : 'none'}"/>
                `;
            }
        });
